...
void main() {
  ...
  test('all oracle games replay correctly', () {
    ...
    for (var gi = 0; gi < games.length; gi++) {
      ...
      var state = GameState.initial(BoardConfig(
        size: boardSize,
        wallsPerPlayer: wpp,
      ));
      ...
      for (final cp in checkpoints) {
        ...
        while (state.turnNumber < expectedPly) {
          ...
          final result =
              GameEngine.apply(state, state.currentPlayer, action!);
          expect(result, isA<SuccessResult>(),
              reason: 'Game $gi (seed=$seed): Action $actionStr failed '
        ...
        expect(_normalizeJson(actualJson), _normalizeJson(expectedState),
        ...
        final actualLegalStrs =
            actualLegal.map((a) => a.toNotation()).toList();
        expect(actualLegalStrs, expectedLegal,
            reason: 'Game $gi (seed=$seed): Legal actions mismatch at ply $expectedPly');
        ...
        for (final cand in candidateActions) {
          final failure =
              ActionValidator.validate(state, state.currentPlayer, cand);
          final code =
              failure == null ? '.' : _failureToCode(failure.name);
          actualCodes.write(code);
        }
        expect(actualCodes.toString(), expectedCandidates,
            reason: 'Game $gi (seed=$seed): Candidate codes mismatch at ply $expectedPly');
      }

      expect(state.turnNumber, (checkpoints.last as Map)['ply'],
...
List<GameAction> _allCandidates(int n) {
  ...
  for (final orient in [WallOrientation.h, WallOrientation.v]) {
    for (var r = 0; r < n; r++) {
      for (var c = 0; c < n; c++) {
        cands.add(GameAction.wall(
            orientation: orient, anchor: Cell(row: r, column: c)));
      }
    }
  }
  cands.add(GameAction.wall(
      orientation: WallOrientation.h, anchor: Cell(row: -1, column: 0)));
  cands.add(GameAction.wall(
      orientation: WallOrientation.v, anchor: Cell(row: 0, column: -1)));
